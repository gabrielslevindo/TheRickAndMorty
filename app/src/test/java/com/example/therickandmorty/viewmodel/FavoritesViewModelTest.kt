...
class FavoritesViewModelTest {
    ...
    fun `checkIsFavorite should update state to true when character is favorite`() {
        runTest {
            ...
            val isFavorite = viewModel.state.map { it.isFavorite }.first { it }
    ...
    @Test
    fun `checkIsFavorite sets isFavorite false when repository throws exception`() {
        runTest {
            repository.shouldThrowOnGetAllFavorites = true
            repository.getAllFavoritesException = RuntimeException("Erro ao carregar favoritos")

            viewModel.onAction(
                FavoritesAction.IsFavorite(characterId = 1),
            )
